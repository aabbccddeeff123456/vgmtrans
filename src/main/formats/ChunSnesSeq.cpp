...
void ChunSnesSeq::LoadEventMap() {
  ...
  EventMap[0xf1] = EVENT_REMOTE_RELEASE_RATE;
...
bool ChunSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_GAIN: {
      ...
      desc
        << L"Arg1: " << (int)arg1;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"GAIN?", desc.str(), CLR_CHANGESTATE, ICON_CONTROL);
      break;
    }

    case EVENT_REMOTE_RELEASE_RATE: {
      uint8_t arg1 = GetByte(curOffset++);
      desc 
        << L"Preset: " << (int)arg1;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Duration Preset", desc.str(), CLR_DURNOTE, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_CONDITIONAL_JUMP: {
      ...
      if (condValue == 0) {
        // infinite loop?
        // Example: Otogirisou - Otogirisou ~Main Theme~
      //  bContinue = false;
      //  break;
      }
