  ...
  while (true) {
    ...
    switch (eventType) {
      ...
      case HEADER_EVENT_09: {
        VGMHeader *aHeader = header->AddHeader(beginOffset, 1, L"Custom Note Vetocity");
        aHeader->AddSimpleItem(beginOffset, 1, L"Event ID");
        ...
        aHeader->AddSimpleItem(curOffset, tableSize, L"Vetocity Pointer");
        curOffset += tableSize;
...
bool HudsonSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    0x79, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0x7f,
            ...
            // percussion note
            AddProgramChangeNoItem(GetByte(parentSeq->PercussionTableAddress + key * 4), true);
            AddVolNoItem(GetByte(parentSeq->PercussionTableAddress + key * 4 + 2));
            AddPanNoItem(PAN_TABLE[30 - GetByte(parentSeq->PercussionTableAddress + key * 4 + 3)]);
            key = GetByte(parentSeq->PercussionTableAddress + key * 4 + 1);
            AddNoteByDur(beginOffset, curOffset - beginOffset, key, vel, dur, L"Percussion Note with Duration");
            prevNoteKey = key;
          }
        }
